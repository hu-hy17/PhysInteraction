...
struct PixelIndexer {
	...
	float3* point_cloud_ptr;
    
public:  
    PixelIndexer(thrust::device_vector<int2>& indexes, thrust::device_vector<float3>& point_cloud) : INVALID(-1), counters_memory(PixelType::SIZE, 0 /*init*/)    {
        this->counters = thrust::raw_pointer_cast(&counters_memory[0]);    
        this->cnstr_indexes  = thrust::raw_pointer_cast(&indexes[0]);
		this->point_cloud_ptr= thrust::raw_pointer_cast(&point_cloud[0]);
    }
