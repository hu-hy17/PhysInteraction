...
namespace cudax {
	...
	//following structure is modified by ZH
	struct ComputeJacobianDataPointCloud : public ComputeJacobianRow {
		const glm::mat3x3& iproj; ///< to compute cloud from depth
		CorrespondencesFinder correspondences_finder;
	...
	public:
		ComputeJacobianDataPointCloud(J_row* J_raw, float* e_raw, bool reweight, bool cal_confidence) :
			ComputeJacobianRow(J_raw, e_raw),
			iproj(cudax::camera_matrix->D_inv_proj_matrix()) {
			point_to_plane = settings->fit3D_point2plane;
			weight = settings->fit3D_weight;
			exclude_outlier=settings->fit3D_exclude_outlier_enable;
			this->reweight = reweight;
			this->cal_conf = cal_confidence;
			debug_store = false;
			correspondences = NULL;
			...
			// init confidence
			// Add by HHY 2022-2-24
			if (cal_confidence)
			{
				thrust::host_vector<int> host_conf(device_confidence->size(), 0);
				*device_confidence = host_conf;
				this->conf = thrust::raw_pointer_cast(device_confidence->data());
			}
		}
		...
		__device__ void assemble_linear_system(int constraint_index, float4 p_sensor) {
			...
			if (exclude_outlier&&(b < 14||b==27) && length(p - q)>10)//8
				return;
			else if (length(p - q) > 25)
				return;
			...
			*e_sub = weight * glm::dot(p - q, n);
			...
			// count data point
			if (cal_conf)
				atomicAdd(&(conf[joint_id]), 1);
		}

		__device__
			void operator()(int index) {

//			int index_temp = _point_index[index];
			float3 point_temp = point_cloud[index];
			float4 p_sensor = make_float4(point_temp.x, point_temp.y, point_temp.z, 0);

			assemble_linear_system(index, p_sensor);
	...
	//this struct is for the second camera
	struct ComputeJacobianDataPointCloudMultiCamera : public ComputeJacobianRow {
		const glm::mat3x3& iproj; ///< to compute cloud from depth
		CorrespondencesFinder correspondences_finder;
	...
	public:
		ComputeJacobianDataPointCloudMultiCamera(J_row* J_raw, float* e_raw, bool reweight) :
			ComputeJacobianRow(J_raw, e_raw),
			iproj(cudax::camera_matrix->D_inv_proj_matrix()) {
			point_to_plane = settings->fit3D_point2plane;
			weight = settings->fit3D_weight;
		...
		__device__ void assemble_linear_system(int constraint_index, float4 p_sensor, const glm::vec3& camera_ray) {
			glm::vec3 p = glm::vec3(p_sensor.x, p_sensor.y, p_sensor.z);
			glm::vec3 q, s;
			glm::ivec3 index;
			int b;

			correspondences_finder.find2(p, camera_ray, b, q, s, index);
		...
		__device__
			...
			glm::vec3 camera_ray = glm::vec3(_camera_ray_dir.x, _camera_ray_dir.y, _camera_ray_dir.z);

			assemble_linear_system(index, p_sensor, camera_ray);
