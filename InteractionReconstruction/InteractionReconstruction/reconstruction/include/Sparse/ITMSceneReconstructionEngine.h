...
_CPU_AND_GPU_CODE_ inline float computeUpdatedVoxelDepthInfo(DEVICEPTR(ITMVoxel) &voxel, const THREADPTR(ORUVector4f) & pt_model, const CONSTPTR(ORUMatrix4f) & M_d,
	const CONSTPTR(ORUVector4f) & projParams_d, float mu, int maxW, pcl::gpu::PtrStepSz<unsigned short> _depth_frame, const CONSTPTR(ORUVector2i) & imgSize)
{
	ORUVector4f pt_camera; ORUVector2f pt_image;
	...
	depth_measure = _depth_frame((int)(pt_image.y + 0.5f), (int)(pt_image.x + 0.5f));// / 1000.0f;// depth[(int)(pt_image.x + 0.5f) + (int)(pt_image.y + 0.5f) * imgSize.x];
	...
	oldF = ITMVoxel::SDF_valueToFloat(voxel.sdf); oldW = voxel.w_depth;
...
_CPU_AND_GPU_CODE_ inline void buildHashAllocAndVisibleTypePP(DEVICEPTR(ORUuchar) *entriesAllocType, DEVICEPTR(ORUuchar) *entriesVisibleType, int x, int y,
	DEVICEPTR(ORUVector4s) *blockCoords, pcl::gpu::PtrStepSz<unsigned short> _depth_frame, ORUMatrix4f invM_d, ORUVector4f projParams_d, float mu, ORUVector2i imgSize,
	float oneOverVoxelSize, const CONSTPTR(ITMHashEntry) *hashTable, float viewFrustum_min, float viewFrustum_max)
{
	...
	ORUVector3f pt_camera_f, point_e, point, direction; ORUVector3s blockPos;

	depth_measure = _depth_frame(y, x);// / 1000.0f;// _depth_frame.ptr(y)[x] / 1000.0f;// _depth_frame[y, x] / 1000.0f;

	if (depth_measure <= 0 || (depth_measure - mu) < 0 || (depth_measure - mu) < viewFrustum_min || (depth_measure + mu) > viewFrustum_max) return;

	//ZH debug
	//blockCoords[x * 640 + y] = Vector4s(1,1,1,0);
#if 1
	...
	for (int i = 0; i < noSteps; i++)
	{
		...
		///ZH:if in the order part
		if (IS_EQUAL3(hashEntry.pos, blockPos) && hashEntry.ptr >= -1)
		{
			//entry has been streamed out but is visible or in memory and visible
			///ZH: 2 indicates the entry has been streamed out but visible, while 1 indicates that the entry is in memory and visible
			entriesVisibleType[hashIdx] = (hashEntry.ptr == -1) ? 2 : 1;

			isFound = true;
		}
		///ZH:if not in the order part
		if (!isFound)
		{
			bool isExcess = false;
			///ZH: if the entry in the ordered part has value, search the entry in the unordered part
			if (hashEntry.ptr >= -1) //seach excess list only if there is no room in ordered part
			{
				///ZH: if has excess list
				...
				///ZH:indicates not in the order part and should be allocated in the excess list: searched or not in the excess list
				isExcess = true;///ZH: This variance and its assignment style must be used in the future to add new entry  
			}

			if (!isFound) //still not found
			{
				///ZH:2 indicates the new entry should be in the excess list, while 1 indicates the new entry in the ordered hash table
				entriesAllocType[hashIdx] = isExcess ? 2 : 1; //needs allocation ZH: and the allocation step will be different for 2 or 1
															  ///ZH:the following sentence will not execute
				if (!isExcess) entriesVisibleType[hashIdx] = 1; ///ZH:this is new entry in the order fragment and it is visible

				blockCoords[hashIdx] = ORUVector4s(blockPos.x, blockPos.y, blockPos.z, 1);
			}
		}
...
_CPU_AND_GPU_CODE_ inline float computeUpdatedVoxelDepthInfo(DEVICEPTR(ITMVoxel) &voxel, const THREADPTR(Vector4f) & pt_model, const CONSTPTR(Matrix4f) & M_d,
...
_CPU_AND_GPU_CODE_ inline void checkPointVisibility(THREADPTR(bool) &isVisible, THREADPTR(bool) &isVisibleEnlarged,
	const THREADPTR(ORUVector4f) &pt_image, const CONSTPTR(ORUMatrix4f) & M_d, const CONSTPTR(ORUVector4f) &projParams_d,
	const CONSTPTR(ORUVector2i) &imgSize)
{
	ORUVector4f pt_buff;
...
_CPU_AND_GPU_CODE_ inline void checkBlockVisibility(THREADPTR(bool) &isVisible, THREADPTR(bool) &isVisibleEnlarged,
	const THREADPTR(ORUVector3s) &hashPos, const CONSTPTR(ORUMatrix4f) & M_d, const CONSTPTR(ORUVector4f) &projParams_d,
	const CONSTPTR(float) &voxelSize, const CONSTPTR(ORUVector2i) &imgSize)
{
	ORUVector4f pt_image;
