...
Camera::Camera(CAMERAMODE mode, float fx, float fy, float cx, float cy, int fps /* =30 */):_mode(mode), _fps(fps){
    ...
    switch(_mode){
        ...
        case Intel:
           _width = 320;
           _height = 240;
		   _focal_length_x = fx / 2;
		   _focal_length_y = fy / 2;
           _zFar = 600;
           _zNear = 100;
            break;            
        default: 
    ...
    auto kinectproj = [=]() { 
		...
		cam_matrix(0, 2) = cx / 2;           /// CameraCenter X
		cam_matrix(1, 2) = cy / 2;           /// CameraCenter Y
        cam_matrix (2, 2) = 1.0;
        return cam_matrix; 
    };
    proj = kinectproj();
...
//    Vector3 wrld = iproj * Vector3( i*depth, (height()-j-1)*depth, depth );
	Vector3 wrld = iproj * Vector3(i*depth, j*depth, depth);
	wrld[1] = -wrld[1];
#else
