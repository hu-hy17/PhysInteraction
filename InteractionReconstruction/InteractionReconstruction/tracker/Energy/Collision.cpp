...
//					E = E + e * e;
					...
					Scalar omega = settings->collision_weight;
					E += omega*fraction*abs(e);
				}
			}
		}
		return E;
	...
	void Collision::track(LinearSystem& system, bool store_error, float &collision_error) {
		// TIMED_SCOPE(timer,"Worker::collision_track");
		if (!settings->collision_enable) return;

		float coli_error=create_collision_constraints();
		collision_compute_update(system, settings->collision_weight);

		if (store_error)
			collision_error = coli_error;
	}
