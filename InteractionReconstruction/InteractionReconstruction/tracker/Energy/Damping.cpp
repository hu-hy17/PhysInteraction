...
namespace energy {
	...
	void Damping::track_full(LinearSystem &system) {
		...
		for (int i = 0; i < num_thetas; ++i) {
			if (model->dofs[i].type == TRANSLATION_AXIS)
				...
				d(i) = settings->rotation_damping;
			if (i == 13 || i == 17 || i == 21 || i == 25||i==9) d(i) = settings->abduction_damping;
			if (i == 16 || i == 20 || i == 24 || i == 28||i==12) d(i) = settings->top_phalange_damping;
