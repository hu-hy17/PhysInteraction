...
void Model::init(int user_name, std::string data_path) {
	...
	//ZH manual update model;
	//resize_model(1.05*1.05*1.05*1.05, 1.0, 1.0);//1.05*1.05*1.03
	//resize_model(1.0, 1.05*1.05*1.05, 1.0);
	//resize_model(1.0, 1.0, 1.05);

	//HHY manual update model
	//resize_model(1.4, 1.0, 1.0);//1.05*1.05*1.03
	//resize_model(1.0, 1.1, 1.0);
	//resize_model(0.9, 1.0, "HandThumb");

	//output the initial phalanges local
	{
		ofstream init_pha_file("../../../../result/init_pha.txt");

		init_pha_file << "root-17" << endl;
		init_pha_file << phalanges[17].init_local << endl << endl;

		init_pha_file << "root-0" << endl;
		init_pha_file << phalanges[0].init_local << endl << endl;

		init_pha_file << "index-13" << endl;
		init_pha_file << phalanges[13].init_local << endl << endl;

		init_pha_file << "middle-10" << endl;
		init_pha_file << phalanges[10].init_local << endl << endl;

		init_pha_file << "ring-7" << endl;
		init_pha_file << phalanges[7].init_local << endl << endl;
	}
}
...
//			printf("i:%d  j:%d  \noffset before:%f  %f  %f \n\n", i, j, phalanges[i].offsets[j][0], phalanges[i].offsets[j][1], phalanges[i].offsets[j][2]);
			phalanges[i].offsets[j] = offset;
//			printf("offset after:%f  %f  %f \n\n", phalanges[i].offsets[j][0], phalanges[i].offsets[j][1], phalanges[i].offsets[j][2]);
...
int Model::compute_rendered_indicator_helper2(int row, int col, Vector3 p, int block, int num_rendered_points, bool display,
	cv::Mat & image, const cv::Mat & sensor_silhouette) {

	if (col < 1 || col >= sensor_silhouette.cols || row < 1 || row >= sensor_silhouette.rows) return num_rendered_points;

	const int window_width = 1;

	int up = row - window_width;
	up = up < 0 ? 0 : up;
	int down = row + window_width;
	down = down > (sensor_silhouette.rows - 1) ? (sensor_silhouette.rows - 1) : down;
	int left = col - window_width;
	left = left < 0 ? 0 : left;
	int right = col + window_width;
	right=right>(sensor_silhouette.cols - 1) ? (sensor_silhouette.cols - 1) : right;

	for(int r=up;r<down;r++)
		for (int c = left; c < right; c++)
		{
			if (sensor_silhouette.at<uchar>(r, c) == 255) return num_rendered_points;
		}

//	if (sensor_silhouette.at<uchar>(row, col) == 255) return num_rendered_points;

	rendered_pixels[num_rendered_points] = row * sensor_silhouette.cols + col;
	rendered_points[3 * num_rendered_points] = p[0];
	rendered_points[3 * num_rendered_points + 1] = p[1];
	rendered_points[3 * num_rendered_points + 2] = p[2];
	rendered_block_ids[num_rendered_points] = block;
...
void Model::compute_rendered_indicator(const cv::Mat & sensor_silhouette, Camera * camera, bool store_silhoutte, int frame_id) {
	bool display = true;
	cv::Mat image;
	if (display) {
...
//		cv::flip(image, image_flip, 1);
		...
		//char curr_silhou_file[512] = { 0 };
		//sprintf(curr_silhou_file, "silhouette/sil_%04d.png", frame_id);
		//cv::imwrite(curr_silhou_file, image);
		cv::waitKey(3);
	}
	/*num_rendered_points = 0;
...
inline Vector2 camera_projection(Eigen::Matrix4f camera_view, float fx, float fy, float cx, float cy, Vector3 s)
{
	Vector4 local = camera_view*Vector4(s[0], -s[1], s[2], 1);
	Scalar x = local[0] / local[2];
	Scalar y = local[1] / local[2];
	x = x*fx + cx;
	y = y*fy + cy;
	return Vector2(x, y);
}

void Model::compute_rendered_indicator2(const cv::Mat & sensor_silhouette, Eigen::Matrix4f camera_view, float fx, float fy, float cx, float cy, bool store_silhoutte, int frame_id) {
	bool display = true;
	cv::Mat image;
	if (display) {
		cv::Scalar color = cv::Scalar(100, 0, 230);
	...
	for (size_t i = 0; i < outline_finder.outline3D.size(); i++) {
		...
		int block = outline_finder.outline3D[i].block;
		if (outline_finder.outline3D[i].indices[1] != RAND_MAX) {
			Vector2 s_image = camera_projection(camera_view, fx, fy, cx, cy, s);
			Vector2 e_image = camera_projection(camera_view, fx, fy, cx, cy, e);
			...
			for (int n = 0; n < num_samples; n++) {
				...
				int col = (int)x;
				int row = (int)y;
				num_rendered_points = compute_rendered_indicator_helper2(row, col, p, block, num_rendered_points, display, image, sensor_silhouette);
			}
		}
		else {
			glm::vec3 c_glm = centers[outline_finder.outline3D[i].indices[0]];
			float r = radii[outline_finder.outline3D[i].indices[0]];
			Vector3 c = Vector3(c_glm[0], c_glm[1], c_glm[2]);
			Vector2 c_image = camera_projection(camera_view, fx, fy, cx, cy, c);
			Vector2 s_image = camera_projection(camera_view, fx, fy, cx, cy, s);
			float r_image = (c_image - s_image).norm();
			Vector3 v1 = s - c;
			Vector3 v2 = e - c;
			...
			float phi;
			Vector2 u = Vector2(1, 0); Vector2 v = Vector2(0, -1);// Vector2 v = Vector2(0, 1);
			Vector3 U = Vector3(1, 0, 0); Vector3 V = Vector3(0, 1, 0);
	...
	//	//		cv::Mat image_flip;
	//	//		cv::flip(image, image_flip, 1);
	//	if (display) cv::imshow("silhouette", image);
	//	/*char curr_silhou_file[512] = { 0 };
	//	sprintf(curr_silhou_file, "silhouette/sil_%04d.png", frame_id);
	//	cv::imwrite(curr_silhou_file, image);*/
	//	cv::waitKey(3);
...
void Model::load_model_from_file() {
	...
	//std::string model_path = "C:/Users/ZH/Desktop/HandReconstruction/capture_sequence13/";
	//read_float_matrix(model_path, "C", centers);
	//read_float_vector(model_path, "R", radii);
	//read_int_matrix(model_path, "B", blocks);
	...
	//ZH output the base initial rotation
	FILE *base_model = fopen("base_model.txt", "w");
	Mat4f transform_mat = Mat4f::Identity();
	transform_mat(0, 0) = -1; transform_mat(2, 2) = -1;
	Mat4f temp_mat;
	//IndexBase
	temp_mat = phalanges[13].init_local * transform_mat;
	for (size_t u = 0; u < d + 1; u++)
		for (size_t v = 0; v < d + 1; v++)
			fprintf(base_model, "%f ", temp_mat(v, u));
	//IndexBottom
	temp_mat = phalanges[14].init_local;
	for (size_t u = 0; u < d + 1; u++)
		for (size_t v = 0; v < d + 1; v++)
			fprintf(base_model, "%f ", temp_mat(v, u));
	//IndexMiddle
	temp_mat = phalanges[15].init_local;
	for (size_t u = 0; u < d + 1; u++)
		for (size_t v = 0; v < d + 1; v++)
			fprintf(base_model, "%f ", temp_mat(v, u));

	//MiddleBase
	temp_mat = phalanges[10].init_local * transform_mat;
	for (size_t u = 0; u < d + 1; u++)
		for (size_t v = 0; v < d + 1; v++)
			fprintf(base_model, "%f ", temp_mat(v, u));
	//MiddleBottom
	temp_mat = phalanges[11].init_local;// temp_mat = transform_mat*phalanges[11].init_local * transform_mat;
	for (size_t u = 0; u < d + 1; u++)
		for (size_t v = 0; v < d + 1; v++)
			fprintf(base_model, "%f ", temp_mat(v, u));
	//MiddleMiddle
	temp_mat = phalanges[12].init_local;// temp_mat = transform_mat*phalanges[11].init_local * transform_mat;
	for (size_t u = 0; u < d + 1; u++)
		for (size_t v = 0; v < d + 1; v++)
			fprintf(base_model, "%f ", temp_mat(v, u));

	//RingBase
	temp_mat = phalanges[7].init_local * transform_mat;
	for (size_t u = 0; u < d + 1; u++)
		for (size_t v = 0; v < d + 1; v++)
			fprintf(base_model, "%f ", temp_mat(v, u));
	//RingBottom
	temp_mat = phalanges[8].init_local;
	for (size_t u = 0; u < d + 1; u++)
		for (size_t v = 0; v < d + 1; v++)
			fprintf(base_model, "%f ", temp_mat(v, u));
	//RingMiddle
	temp_mat = phalanges[9].init_local;
	for (size_t u = 0; u < d + 1; u++)
		for (size_t v = 0; v < d + 1; v++)
			fprintf(base_model, "%f ", temp_mat(v, u));

	//PinkyBase
	temp_mat = phalanges[4].init_local * transform_mat;
	for (size_t u = 0; u < d + 1; u++)
		for (size_t v = 0; v < d + 1; v++)
			fprintf(base_model, "%f ", temp_mat(v, u));
	//PinkyBottom
	temp_mat = phalanges[5].init_local;
	for (size_t u = 0; u < d + 1; u++)
		for (size_t v = 0; v < d + 1; v++)
			fprintf(base_model, "%f ", temp_mat(v, u));
	//PinkyMiddle
	temp_mat = phalanges[6].init_local;
	for (size_t u = 0; u < d + 1; u++)
		for (size_t v = 0; v < d + 1; v++)
			fprintf(base_model, "%f ", temp_mat(v, u));
...
void Model::write_centers(std::ofstream &centers_out)
{
	for (int i = 0; i < 20; i++)
		centers_out << centers[i][0] << ' ' << centers[i][1] << ' ' << centers[i][2] << ' ';

	centers_out << centers[25][0] << ' ' << centers[25][1] << ' ' << centers[25][2] << ' '; 
	centers_out << centers[33][0] << ' ' << centers[33][1] << ' ' << centers[33][2] << ' ';
	centers_out << centers[26][0] << ' ' << centers[26][1] << ' ' << centers[26][2] << ' ';
	centers_out << centers[27][0] << ' ' << centers[27][1] << ' ' << centers[27][2] << ' ';
	centers_out << centers[24][0] << ' ' << centers[24][1] << ' ' << centers[24][2] << ' ';
	centers_out << centers[23][0] << ' ' << centers[23][1] << ' ' << centers[23][2] << ' ';
	centers_out << centers[22][0] << ' ' << centers[22][1] << ' ' << centers[22][2] << ' ';
	centers_out << centers[21][0] << ' ' << centers[21][1] << ' ' << centers[21][2] << ' ';
	centers_out << centers[20][0] << ' ' << centers[20][1] << ' ' << centers[20][2] << ' ';
	centers_out << centers[31][0] << ' ' << centers[31][1] << ' ' << centers[31][2] << ' ';
	centers_out << centers[30][0] << ' ' << centers[30][1] << ' ' << centers[30][2] << ' ';
	centers_out << centers[29][0] << ' ' << centers[29][1] << ' ' << centers[29][2] << ' ';
	centers_out << centers[28][0] << ' ' << centers[28][1] << ' ' << centers[28][2] << std::endl;
}

void Model::write_radius(std::ofstream &radius_out)
{
	for (int i = 0; i < 20; i++)
		radius_out << radii[i] << ' ';

	radius_out << radii[25] << ' ' << radii[33] << ' ' << radii[26] << ' ' << radii[27] << ' ' << radii[24] << ' ';
	radius_out << radii[23] << ' ' << radii[22] << ' ' << radii[21] << ' ' << radii[20] << ' ' << radii[31] << ' ';
	radius_out << radii[30] << ' ' << radii[29] << ' ' << radii[28] << std::endl;
}
...
void Model::resize_model(float uniform_scaling_factor, float width_scaling_factor, const std::string& name_prefix)
{
	Mat3d scaling_matrix = Mat3d::Identity();
	scaling_matrix(0, 0) = uniform_scaling_factor * width_scaling_factor;
	scaling_matrix(1, 1) = uniform_scaling_factor;
	...
	for (size_t i = 0; i < num_phalanges; i++) {
		if (phalanges[i].name.substr(0, name_prefix.size()) == name_prefix)
		{
			phalanges[i].init_local.col(3).segment(0, 3) = scaling_matrix.cast <float>() * phalanges[i].init_local.col(3).segment(0, 3);
			for (size_t j = 0; j < phalanges[i].attachments.size(); j++) {
				phalanges[i].offsets[j] = scaling_matrix * phalanges[i].offsets[j];
...
std::vector<unsigned char> Model::get_phalange_centerId()
{
	std::vector<unsigned char> phalange_centerIds;

	for (int i = 0; i < 17; i++)
	{
		phalange_centerIds.push_back(phalanges[i].center_id);
	}

	return phalange_centerIds;
}

std::vector<Eigen::Matrix4f> Model::get_phalange_global_mat()
{ 
	std::vector<Eigen::Matrix4f> phalange_global;

	for (int i = 0; i < 17; i++)
	{
		phalange_global.push_back(phalanges[i].global);
	}

	return phalange_global;
}