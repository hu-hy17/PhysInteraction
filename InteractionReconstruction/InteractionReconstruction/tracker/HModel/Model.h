...
public:
	...
	int compute_rendered_indicator_helper2(int row, int col, Vector3 p, int block, int num_rendered_points, bool display,
		cv::Mat & image, const cv::Mat & sensor_silhouette);

	void compute_rendered_indicator(const cv::Mat & sensor_silhouette, Camera * camera, bool store_silhoutte, int frame_id);

	void compute_rendered_indicator2(const cv::Mat & sensor_silhouette,Eigen::Matrix4f camera_view, float fx, float fy, float cx, float cy,bool store_silhoutte, int frame_id);
	...
	void resize_model(float uniform_scaling_factor, float width_scaling_factor, const std::string& name_prefix);
	...
	void write_centers(std::ofstream &centers_out);

	void write_radius(std::ofstream &radius_out);
	...
	std::vector<unsigned char> get_phalange_centerId();
	std::vector<Eigen::Matrix4f> get_phalange_global_mat();
};