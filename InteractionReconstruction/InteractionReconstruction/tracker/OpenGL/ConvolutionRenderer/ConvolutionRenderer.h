...
class ConvolutionRenderer {	
	...
	struct Camera {
		...
		void setup2(GLuint program_id, Eigen::Matrix4f projection, Eigen::Matrix4f view_mat) {
			//View_projection matrix is equal to the bellow plus small correction terms
			/*float F = 287.26;
			float w = 320;
			float h = 240;
			...
			glUniformMatrix4fv(glGetUniformLocation(program_id, "model"), ONE, false, model.data());
			glUniformMatrix4fv(glGetUniformLocation(program_id, "view"), ONE, false, view_mat.data());
			glUniformMatrix4fv(glGetUniformLocation(program_id, "projection"), ONE, false, projection.data());
		}

		void setup3(GLuint program_id, Eigen::Matrix4f projection, Eigen::Matrix4f view_mat, Eigen::Vector3f camera_center) {
			//View_projection matrix is equal to the bellow plus small correction terms
...
public:
	...
	void render2(Eigen::Matrix4f view_mat);

	void render3(Eigen::Matrix4f camera_projection, Eigen::Matrix4f view_mat, Eigen::Vector3f camera_center);
