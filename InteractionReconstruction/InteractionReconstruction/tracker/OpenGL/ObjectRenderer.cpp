...
void ObjectRenderer::set_uniform(const char* name, const Eigen::Vector3f& value)
{
	assert(program.isLinked());
	program.bind();
	GLuint id = glGetUniformLocation(program.programId(), name);
...
void ObjectRenderer::set_uniform_program(QGLShaderProgram& program, const char* name, const Eigen::Matrix4f& value) {
	assert(program.isLinked());
	program.bind();
	GLuint id = glGetUniformLocation(program.programId(), name);
	if (id == -1)
...
void ObjectRenderer::set_uniform_program(QGLShaderProgram& program, const char* name, const Eigen::Vector3f& value)
{
	assert(program.isLinked());
	program.bind();
	GLuint id = glGetUniformLocation(program.programId(), name);
...
void ObjectRenderer::set_uniform_program(QGLShaderProgram& program, const char* name, float value)
{
	assert(program.isLinked());
	program.bind();
	GLuint id = glGetUniformLocation(program.programId(), name);
