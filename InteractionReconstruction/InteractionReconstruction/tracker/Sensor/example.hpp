...
public:
	...
	void on_key_record(bool is_record) {record_flag = is_record; };
	void on_key_escape(bool is_escape) { escape_flag = is_escape; };
        ...
        : _width(width), _height(height)
    {
        ...
        glfwSetKeyCallback(win, [](GLFWwindow * win, int key, int scancode, int action, int mods)
        {
			...
			{
				s->on_key_record(true);
			}
			if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
			{
				s->on_key_escape(true);
			}
        });
    }
	...
	bool is_record() { return record_flag;	}
	bool is_escape() { return escape_flag; }
