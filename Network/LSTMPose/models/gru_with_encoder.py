...
def pred_net(features,
    ...
    with tf.variable_scope('pred_net'):
        ...
        if lstm_num_layer == 1:
            lstm_cell = tf.nn.rnn_cell.GRUCell(lstm_num_unit)
        else:
            lstm_cell = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.GRUCell(lstm_num_unit) for _ in range(lstm_num_layer)])
